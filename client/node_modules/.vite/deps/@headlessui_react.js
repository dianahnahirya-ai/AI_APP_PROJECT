...
function isAppendWithTrim(prevCount, nextCount, getPreviousKey, getNextKey) {
  ...
  for (let i8 = 0; i8 < retainedCount; i8++) {
    if (getNextKey(i8) !== getPreviousKey(removedCount + i8)) return false;
  }
  for (let i8 = retainedCount; i8 < nextCount; i8++) {
    if (removedKeys.has(getNextKey(i8))) return false;
  }
  return true;
}
...
function useVirtualizerBase({
  ...
  const [instance] = React.useState(() => {
    ...
    const measureElement2 = v6.measureElement;
    v6.measureElement = (node) => {
